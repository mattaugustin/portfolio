...
parameter_table      <-  read_csv("portfolio/residuals/NDSHA_parameters_unique.csv", trim_ws = FALSE)  %>% as.data.frame()
...
write.csv(parameter_table_residual_directivity , "portfolio/residuals/results/NDSHA_predictions_aggregated.csv", row.names = FALSE)
...
SAC_table   <- read_csv("portfolio/residuals/SAC_parameters_unique.csv", trim_ws = FALSE)  %>% as.data.frame()
...
NDSHA_agg <-  read_csv("portfolio/residuals/results/NDSHA_predictions_aggregated.csv", trim_ws = FALSE)  %>% as.data.frame()
...
ggsave("portfolio/residuals/img/NDSHA_vs_SAC_median.png", width = 16, height = 18, units = "cm", device = "png")
...
## CREATE AND ARRANGE RESIDUAL TABLE
...
## RE-ARRANGE RESIDUAL TABLE IN A FORMAT MORE SUITABLE FOR LATER PLOTTING
residual_NDSHA_SAC <- 
...
#####
###############################################################################
###         RESIDUAL PLOTTING  & ASSESSING IMPACT OF VARIOUS SETTINGS       ###
###############################################################################

# NB : the plot theme style, "port_residuals_style", is defined at the end of the script

residual_NDSHA_SAC  <-  read_csv("portfolio/residuals/results/residuals_bin_NDSHA_SAC.csv", trim_ws = FALSE)  %>% as.data.frame()
...
ggsave("portfolio/residuals/img/Allgeo_vs_Rock.png", width = 16, height = 13, units = "cm", device = "png")
...
ggsave("portfolio/residuals/img/10_vs_35Hz.png", width = 16, height = 13, units = "cm", device = "png")
...
ggsave("portfolio/residuals/img/directivity_impact.png", width = 16, height = 13, units = "cm", device = "png")
...
ggplot( ) +
  ...
  scale_x_continuous( breaks = seq(0,40,4) , limits = c(0, 20) , expand = c(0,0) ) +  
  ...
  scale_fill_manual( values = c( "rock"= "grey40"  , "soil" = new_colours11(8)[6]   )  ) 


ggsave("portfolio/residuals/img/All_geo_histogram1.png", width = 18, height = 13, units = "cm", device = "png")
...
ggsave("portfolio/residuals/img/Filtering_histogram.png", width = 18, height = 13, units = "cm", device = "png")
...
ggsave("portfolio/residuals/img/Directivity_histogram1.png", width = 18, height = 13, units = "cm", device = "png")
...
ggsave("portfolio/residuals/img/Directivity_histogram2.png", width = 18, height = 13, units = "cm", device = "png")
...
residual_NDSHA_SAC %>% filter(. ,  quantile == "median" , 
  ...
  mutate( . , directivity_plot = case_when(.$directivity == "bil_dir000" ~ "0 °"   , 
                                           .$directivity == "bil_dir090" ~ "90 °"  ,
                                           .$directivity == "bil_dir180" ~ "180 °" )) %>%     
  mutate(., residual_bin_manual = cut( residual_amplitude , breaks = seq(-2,2,0.5) , right = T ))  %>%  
...
ggsave("portfolio/residuals/img/Directivity_histogram3.png", width = 18, height = 18, units = "cm", device = "png")
...
NDSHA_table   <-   read_csv("portfolio/residuals/NDSHA_parameters_unique.csv", trim_ws = FALSE)  %>% as.data.frame()
...
ggsave("portfolio/residuals/img/NDSHA_vs_SAC_swarm.png", width = 16, height = 18, units = "cm", device = "png")
