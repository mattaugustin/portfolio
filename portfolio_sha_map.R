...
#
regio_pga_df <- read_csv("C:/ndsha_maps/gmpe_1/bigmap/catalogue_BGS2020/map_BGSstp/newzones/lomesh_m4_deptite_g11/dataset/regiomap_pga_uk_example.csv") %>% as.data.frame()
...
###
ukmap_spdf      <-  rgdal::readOGR( dsn =  "dataset/ukmap.shp", stringsAsFactors = F , ) 
...
###
test_ukmap_BNG_df      <-  read.csv("C:/ndsha_maps/seismicmaps/dataset/img/Kriging_data/UKmap_BNG_df.csv" , as.is = 7 , stringsAsFactors =  TRUE) %>% as.data.frame() 
test_IsleOfMan_BNG_df  <-  read.csv("C:/ndsha_maps/seismicmaps/dataset/img/Kriging_data/IsleOfMan_df.csv", as.is = 7 ) %>% as.data.frame()
class(test_IsleOfMan_BNG_df)
...
ggplot() + 
  ...
  geom_polygon(test_ukmap_BNG_df[ ] , mapping = aes(long, lat, group = group_char), colour = "black", fill = "white", size = .2) +
  geom_polygon(test_IsleOfMan_BNG_df                         , mapping = aes(long, lat, group = group), colour = "black", fill = "white" , size = .2) +
  geom_point(regio_pga_df , mapping = aes( x = longitude_easting, y = latitude_northing, fill = pga_g_bin) , col = "white", shape = 21) +
  geom_polygon(test_ukmap_BNG_df[test_ukmap_BNG_df$FID == "UK", ] , mapping = aes(long, lat, group = group_char), colour = "black", fill = NA, size = .2) +
  #
...
ggsave(  "dataset/img/regiomap_pga_original.png" , 
...
ggsave(  "dataset/img/regiomap_pga_kriged.png" , 
         plot = ggplot2::last_plot() , width = 21   , height = 21 , units = "cm" , type = "cairo") 
