...
##  clean a list and multiple-level nested lists from empty elements
...
remove.empty <- function(l){ lapply(l, function(l){l[lengths(l)>0]}) }
...
path_signal <- expand.grid( directivity  , file_list_signal , real_nb , struct_nb , station_nb  )  %>% 
      dplyr::rename(. , "directivity" = "Var1", "event"="Var2"  , "real"="Var3"  ,  "structure"="Var4" , "stn"="Var5" )
...
path_signal$accel_ew     <- paste(  data_directory, "bil", path_signal$directivity , "/" , path_signal$event  , "/real" , path_signal$real , "/signal/ukr",
                                    path_signal$structure,"f2f2.sew.0000",path_signal$stn ,".plt",  sep="") 
path_signal$accel_ns     <- paste(  data_directory, "bil", path_signal$directivity , "/"  ,path_signal$event  , "/real" , path_signal$real , "/signal/ukr",
                                    path_signal$structure,"f2f2.sns.0000",path_signal$stn ,".plt",  sep="")

## quake files generated by DWN mode
path_signal_bis$accel_ew <- paste(  data_direc, "bil", path_signal_bis$directivity , "/" , path_signal_bis$event  , "/real" , path_signal_bis$real , "/signal/ukr",
                                    path_signal_bis$structure , "puf2f2.sew.0000" , path_signal_bis$stn , ".plt",  sep="")
path_signal_bis$accel_ns <- paste(  data_direc, "bil", path_signal_bis$directivity , "/" , path_signal_bis$event  , "/real" , path_signal_bis$real , "/signal/ukr",
...
path_signal$chan <- str_sub( path_signal$event , start = nchar(path_signal$event %>% as.character()) - 4  , end = nchar(path_signal$event %>% as.character())    )

path_signal$exp_name_ew  <-  paste0(  path_signal$event , "_dir", path_signal$directivity , "_real" ,  path_signal$real , 
                                     "_stn" , path_signal$stn , "_" , as.numeric(path_signal$struct),"ew") 
path_signal$exp_name_ns  <-  paste0(  path_signal$event , "_dir", path_signal$directivity , "_real" ,  path_signal$real , 
...
stacked_list_ndsha_acc <- list( dir000 = list(), dir090 = list() , dir180 = list() )
# fill each directivity with event
for (i in 1:length(stacked_list_ndsha_acc)) {
  stacked_list_ndsha_acc[[i]] <- list(   dudley = list()  , rasen = list() , swansea = list()     )
  #
  # fill each event with sampling rate (channel)
  for (j in 1:length(stacked_list_ndsha_acc[[i]])) {
    stacked_list_ndsha_acc[[i]][[j]] <- list( Bchan = list(),  Hchan = list() )
    #
...
# list_test  <-  stacked_list_ndsha_acc[lapply(stacked_list_ndsha_acc, length) >0]
list_test_c  <-  remove.empty(  stacked_list_ndsha_acc )
...
stacked_ndsha_acc_tables <-  list(   dir000 = list()  ,  dir090 = list() , dir180 = list() ) 
parameter_table          <-  list(   dir000 = list()  ,  dir090 = list() , dir180 = list() )     
...
for (i in 1:length(stacked_list_ndsha_acc)) {
  ...
  stacked_ndsha_acc_tables[[i]] <- list(  dudley = list() , rasen = list() , swansea = list()    ) 
  parameter_table[[i]]          <- list(  dudley = list() , rasen = list() , swansea = list()    ) 
  #
  for (j in 1:length(stacked_list_ndsha_acc[[i]])) {
    #
    # define CHANNEL at k level
    stacked_ndsha_acc_tables[[i]][[j]] <- list(  Bchan = list() , Hchan = list() )
    parameter_table[[i]][[j]]          <- list(  Bchan = list() , Hchan = list() )
    #
    for (k in 1:length(stacked_list_ndsha_acc[[i]][[j]])) {
      #  
      ...
      for (l in 1:length(stacked_list_ndsha_acc[[i]][[j]][[k]])) {
        #
        #  # verify whether list is empty at REALISATION level
        if( length(stacked_list_ndsha_acc[[i]][[j]][[k]][[l]]) != 0 ) {  
          #
          stacked_list_subset_ew <- c () ; stacked_list_subset_ns <- c() ; srate <- c() ; time_ew <- c() ; stat_id <- c() ;
          maxHe <- c() ; maxHn <- c() ; maxAccGM <- c() ; maxAccRes <- c() ; maxEN <- c() ; max_spacHn <- c() ;
          ...
          for (m in 1:(length(stacked_list_ndsha_acc[[i]][[j]][[k]][[l]])) ){
            ...
            srate                    <- time_ew[2]
            #
            maxHe[m] <- round( max(abs(stacked_list_subset_ew )) , 3 )
            maxHn[m] <- round( max(abs(stacked_list_subset_ns )) , 3 )
            maxEN[m] <- max (maxHe[m] , maxHn[m] )
            #
            stat_id[m] <- str_sub( names(stacked_list_ndsha_acc[[i]][[j]][[k]][[l]])[m] , nchar(names(stacked_list_ndsha_acc[[i]][[j]][[k]][[l]])[m]  ) - 6 ,
          ...
          stacked_ndsha_acc_tables[[i]][[j]][[k]][[l]] <-  cbind.data.frame(  stat_id, maxHe , maxHn , maxEN , maxAccGM , maxAccRes ,
          ...
          stacked_ndsha_acc_tables[[i]][[j]][[k]][[l]]$chan        <- names(stacked_ndsha_acc_tables[[i]][[j]])[k]
...
parameter_table  <-  parameter_table %>% relocate( . , c(directivity,event,chan,real) , .before = stat_id  )
