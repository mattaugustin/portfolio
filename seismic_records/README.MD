...
As shown, earthquake-related signal (visible via the amplitude) lasting a few minutes may be included as part of 1-hour-long SAC file. The long file duration has a direct impact on file size as well computer-resources in later spectral acceleration calculations, so it is necessary to reduce the signal to its significant part within a time window bracketing the earthquake.        
Unfortunately, signal clipping is not always possible when metadata for some SAC files are inconsistent. I therefore identified and corrected SAC metadata for these files where inconsistencies exist, using PZ files and the "earthquake info" table,
